...
import com.karasluo.libcommon.R;
import com.karasluo.libcommon.utils.RxBus;
import com.karasluo.libcommon.widget.ProgressDialog;
...
//                                showToast((String) busEvent.getMsg());
    ...
    /**
     * 路由跳转activity
     * @param path path必须为两级目录，如:"/test1/test2",默认分组为"test1"
     */
    public void switchActivity(String path){
        ActivityOptionsCompat optionsCompat=ActivityOptionsCompat.makeCustomAnimation(
                AppActivity.this,
                R.anim.common_push_right_in,
    ...
    public void addFragmentWithAnimations(AppFragment fragment){
        if(fragment!=null){
            ...
            getSupportFragmentManager().beginTransaction()
                    .setCustomAnimations(R.anim.common_push_right_in,R.anim.common_push_left_out,
                            R.anim.common_push_left_in,R.anim.common_push_right_out)
                    .add(getFragmentContainerId(),fragment,fragment.getTAG())
                    .addToBackStack(((Object)fragment).getClass().getSimpleName())
                    .hide(lastFragment)
                    .commit();
    ...
    /**
     * 调用系统默认浏览器
     * @param netAddress 访问地址
     */
    public void openBrowser(String netAddress){
